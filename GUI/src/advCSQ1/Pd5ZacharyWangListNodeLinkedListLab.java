...
public class Pd5ZacharyWangListNodeLinkedListLab
{
   ...
   {//This might be cheating
      ...
      for(ListNode<Integer> temp = head; temp != null; temp = temp.getNext())
    	  vals.add(0, temp.getValue());//create an arraylist with the values stored in reverse
      ListNode<Integer> temp = head;
      for(int i = 0; temp != null; temp = temp.getNext(), i++)
    	  temp.setValue(vals.get(i));//use that arraylist to fill everything back in
      return head;
   }
